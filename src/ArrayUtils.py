...
def array_copy(
    ...
    """Copies subset of one array into another."""
    if size is None:
        size = len(source)
    destination[destOffset : destOffset + size] = source[srcOffset : srcOffset + size]


def array_trim(source: bytearray) -> bytearray:
    """Remove all trailing 0s from a bytearray."""
    i = len(source) - 1
...
def encode_name(name: str) -> bytearray:
    """Use the charmap to encode a str into hex bytes."""
...
def decode_name(name: bytearray) -> str:
    """Use the charmap to decode a bytearray into a str."""
    decoded_name = ""
    for i in name:
        poke_char = i & 0xFF
        c = ""
...
def pad_array(source: bytearray, max: int) -> bytearray:
    size = 1
    while 2 << size < max:
        size += 1
